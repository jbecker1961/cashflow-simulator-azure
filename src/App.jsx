...
const LOCAL_SK = "cashflow_sim_v3";
const MX = 15;
...
function gD(){return{
  scenarioMode:"buy",
  /* buy fields */
  homePrice:0,downPayment:0,mortgageType:"30fixed",rate:0,hoa:0,pmi:0,taxMode:"dollar",annualTax:0,taxRatePct:0,annualInsurance:0,addons:[],
  /* rent fields */
  monthlyRent:0,renterInsurance:0,petFee:0,rentLandscaping:0,rentHoa:0,rentParking:0,rentAddons:[],
  /* current house disposition */
  currentHouseMode:"sell",
  /* sell mode */
  oldMortgage:0,
  /* rent-out mode */
...
function cFS(s){
  ...
  if((s.scenarioMode||"buy")==="buy"){
    loan=Math.max(0,(s.homePrice||0)-(s.downPayment||0));
    ...
    pi=s.mortgageType==="30fixed"?fPI(loan,s.rate||0,30):ioP(loan,s.rate||0);
    pIO=s.mortgageType!=="30fixed"?fPI(loan,s.rate||0,23):null;
    nH=pi+mT+mI+(s.hoa||0)+(s.pmi||0)+aT;
    ...
    const rA=sm(s.rentAddons);
    nH=(s.monthlyRent||0)+(s.renterInsurance||0)+(s.petFee||0)+(s.rentLandscaping||0)+(s.rentHoa||0)+(s.rentParking||0)+rA;
    newBreakdown=[{label:"Rent",amount:s.monthlyRent||0},{label:"Renter's Insurance",amount:s.renterInsurance||0},{label:"Pet Fee",amount:s.petFee||0},{label:"Landscaping",amount:s.rentLandscaping||0},{label:"HOA",amount:s.rentHoa||0},{label:"Parking",amount:s.rentParking||0},{label:"Add-ons",amount:rA}];
  }
  /* offsets */
  ...
  const tO=houseOff+(s.stateTaxSavings||0)+fO+cO+oO;
  const d=nH-tO;
  const eBC=CATS.map(c=>({...c,total:sm((s.expenses||{})[c.key]||[])}));
  const tOn=eBC.reduce((a,c)=>a+c.total,0);
  /* current outflows: if selling, old mortgage is expense; if renting out, current mortgage is expense; if empty, current mortgage */
  let curHousingExp=0;
  if(chm==="sell")curHousingExp=s.oldMortgage||0;
  else curHousingExp=s.currentMortgage||s.oldMortgage||0;
  const cTO=curHousingExp+tOn+(s.stateTaxSavings||0);
...
function eqRent(tD,sB){let lo=0,hi=1e5;for(let i=0;i<60;i++){const m=(lo+hi)/2;if(cFS({...sB,monthlyRent:m}).delta<tD)lo=m;else hi=m;}return Math.round((lo+hi)/2);}

function loadLocal(){try{const r=localStorage.getItem(LOCAL_SK);if(r)return JSON.parse(r);const old=localStorage.getItem("cashflow_sim_v2");return old?JSON.parse(old):[];}catch{return[];}}
...
function xls(scs){if(!scs.length)return;const h=["Scenario","Mode","Home Price","Rent","Down Payment","Loan","Rate %","Type","Housing Total","House Disp","House Offset","State Tax Sav","FICA Sav","Car Payoffs","Other Off","Total Off","Delta","Income","Ongoing Exp","New Outflows","New CF","Cur Outflows","Cur CF"];const rows=scs.map(sc=>{const c=cFS(sc.state);const m=sc.state.scenarioMode||"buy";return[sc.name,m,m==="buy"?sc.state.homePrice:"",m==="rent"?sc.state.monthlyRent:"",m==="buy"?sc.state.downPayment:"",m==="buy"?c.loan:"",m==="buy"?sc.state.rate:"",m==="buy"?(sc.state.mortgageType==="30fixed"?"30yr":"IO"):"",Math.round(c.newHousing),sc.state.currentHouseMode||"sell",Math.round(c.houseOff),sc.state.stateTaxSavings,Math.round(c.ficaOff),Math.round(c.carOffT),Math.round(c.otherOffT),Math.round(c.totalOff),Math.round(c.delta),sc.state.monthlyIncome,Math.round(c.totalOngoing),Math.round(c.newTotalOut),Math.round(c.newCashflow),Math.round(c.currentTotalOut),Math.round(c.currentCashflow)];});const csv=[h,...rows].map(r=>r.map(c=>`"${c}"`).join(",")).join("\n");const b=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="cashflow_scenarios.csv";a.click();}
function Donut({items,size=140,thickness=20,label,amount}){const total=items.reduce((s,i)=>s+i.amount,0);const colors=["#2563eb","#7c3aed","#16a34a","#d97706","#dc2626","#ec4899","#0891b2","#64748b","#f97316"];if(total===0)return null;const r=(size-thickness)/2,cx=size/2,cy=size/2;let cumAngle=-90;const arcs=items.filter(i=>i.amount>0).map((item,idx)=>{const pct=item.amount/total;const angle=pct*360;const startAngle=cumAngle;cumAngle+=angle;const endAngle=cumAngle;const largeArc=angle>180?1:0;const s1=Math.cos(startAngle*Math.PI/180)*r;const s2=Math.sin(startAngle*Math.PI/180)*r;const e1=Math.cos(endAngle*Math.PI/180)*r;const e2=Math.sin(endAngle*Math.PI/180)*r;const d=`M ${cx+s1} ${cy+s2} A ${r} ${r} 0 ${largeArc} 1 ${cx+e1} ${cy+e2}`;return<path key={idx} d={d} fill="none" stroke={colors[idx%colors.length]} strokeWidth={thickness} strokeLinecap="butt" style={{transition:"all .5s ease"}}/>;});return<div style={{position:"relative",width:size,height:size,flexShrink:0}}><svg width={size} height={size}>{arcs}</svg><div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,color:P.textMute,fontWeight:500}}>{label}</span><span style={{fontSize:18,fontWeight:700,color:P.text,fontFamily:"'Fraunces',serif"}}>{amount}</span></div></div>;}
function HBar({items,total}){const colors=["#2563eb","#7c3aed","#16a34a","#d97706","#dc2626","#ec4899","#0891b2","#64748b","#f97316"];const t=Math.abs(total)||1;return<div style={{display:"flex",flexDirection:"column",gap:6}}><div style={{display:"flex",borderRadius:8,overflow:"hidden",height:10,background:P.bgInput}}>{items.filter(i=>i.amount>0).map((item,idx)=><div key={idx} style={{width:`${(item.amount/t)*100}%`,background:colors[idx%colors.length],transition:"width .5s ease",minWidth:3}}/>)}</div><div style={{display:"flex",flexWrap:"wrap",gap:"4px 14px"}}>{items.filter(i=>i.amount>0).map((item,idx)=><span key={idx} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:P.textSec}}><span style={{width:8,height:8,borderRadius:3,background:colors[idx%colors.length],display:"inline-block"}}/>{item.label} <strong>{money(item.amount)}</strong></span>)}</div></div>;}
...
function NumInput({value,onChange,style:sx}){const[d,setD]=useState(fC(value));const[f,setF]=useState(false);const ref=useRef(null);useEffect(()=>{if(!f)setD(fC(value));},[value,f]);return<input ref={ref} type="text" inputMode="decimal" value={d} onChange={e=>{if(/^-?[\d.,]*$/.test(e.target.value))setD(e.target.value);}} onFocus={()=>{setF(true);if(value===0)setD("");else setD(sC(d));setTimeout(()=>ref.current?.select(),0);}} onBlur={()=>{setF(false);const p=parseFloat(sC(d));if(isNaN(p)||d.trim()===""){onChange(0);setD("0");}else{onChange(p);setD(fC(p));}}} style={{background:"transparent",border:"none",outline:"none",color:P.text,fontSize:15,fontFamily:"'Plus Jakarta Sans',sans-serif",padding:"0 12px",width:"100%",height:"100%",fontWeight:500,...sx}}/>;}
function Field({label,value,onChange,prefix,suffix,hint,small}){return<div style={{display:"flex",flexDirection:"column",gap:5,flex:small?"0 0 auto":1}}><label style={{fontSize:12,fontWeight:600,color:P.textSec,letterSpacing:"0.02em"}}>{label}</label><div style={{display:"flex",alignItems:"center",background:P.bgInput,borderRadius:10,border:`1.5px solid ${P.border}`,overflow:"hidden",height:44,transition:"border-color .2s"}} onFocus={e=>e.currentTarget.style.borderColor=P.accent} onBlur={e=>e.currentTarget.style.borderColor=P.border}>{prefix&&<span style={{padding:"0 0 0 12px",color:P.textMute,fontSize:15,fontWeight:600}}>{prefix}</span>}<NumInput value={value} onChange={onChange}/>{suffix&&<span style={{padding:"0 12px 0 0",color:P.textMute,fontSize:13,whiteSpace:"nowrap",fontWeight:500}}>{suffix}</span>}</div>{hint&&<span style={{fontSize:11,color:P.textMute}}>{hint}</span>}</div>;}
function Toggle({options,value,onChange}){return<div style={{display:"flex",width:"100%",background:P.bgInput,borderRadius:10,border:`1.5px solid ${P.border}`,overflow:"hidden",padding:3}}>{options.map(o=><button key={o.value} onClick={()=>onChange(o.value)} style={{flex:1,padding:"8px 12px",fontSize:12,fontWeight:600,fontFamily:"'Plus Jakarta Sans'",border:"none",cursor:"pointer",borderRadius:8,transition:"all .25s",background:value===o.value?P.accent:"transparent",color:value===o.value?"#fff":P.textSec,boxShadow:value===o.value?"0 1px 3px rgba(37,99,235,.3)":"none"}}>{o.label}</button>)}</div>;}
function Toggle3({options,value,onChange}){return<div style={{display:"flex",width:"100%",background:P.bgInput,borderRadius:10,border:`1.5px solid ${P.border}`,overflow:"hidden",padding:3,gap:2}}>{options.map(o=><button key={o.value} onClick={()=>onChange(o.value)} style={{flex:1,padding:"7px 8px",fontSize:11,fontWeight:600,fontFamily:"'Plus Jakarta Sans'",border:"none",cursor:"pointer",borderRadius:7,transition:"all .25s",background:value===o.value?P.accent:"transparent",color:value===o.value?"#fff":P.textSec,boxShadow:value===o.value?"0 1px 3px rgba(37,99,235,.3)":"none",lineHeight:1.2,textAlign:"center"}}>{o.label}</button>)}</div>;}
function DynList({items,setItems,defaultName,addLabel}){const add=()=>setItems([...items,{id:uid(),name:defaultName,amount:0}]);const rm=id=>setItems(items.filter(i=>i.id!==id));const up=(id,k,v)=>setItems(items.map(i=>i.id===id?{...i,[k]:v}:i));return<div style={{display:"flex",flexDirection:"column",gap:8}}>{items.map(item=><div key={item.id} style={{display:"flex",gap:8,alignItems:"center"}}><input value={item.name} onChange={e=>up(item.id,"name",e.target.value)} onFocus={e=>{if(item.name===defaultName)e.target.select();}} style={{flex:1,background:P.bgInput,border:`1.5px solid ${P.border}`,borderRadius:10,color:P.text,fontSize:13,fontFamily:"'Plus Jakarta Sans'",padding:"9px 12px",outline:"none",fontWeight:500}}/><div style={{display:"flex",alignItems:"center",background:P.bgInput,borderRadius:10,border:`1.5px solid ${P.border}`,overflow:"hidden",width:120,height:40}}><span style={{padding:"0 0 0 10px",color:P.textMute,fontSize:14,fontWeight:600}}>$</span><NumInput value={item.amount} onChange={v=>up(item.id,"amount",v)} style={{fontSize:13,padding:"0 8px"}}/></div><button onClick={()=>rm(item.id)} style={{background:"none",border:"none",color:P.textMute,cursor:"pointer",fontSize:20,lineHeight:1,padding:4,borderRadius:6,transition:"all .2s"}} onMouseEnter={e=>{e.target.style.color=P.red;e.target.style.background=P.redBg;}} onMouseLeave={e=>{e.target.style.color=P.textMute;e.target.style.background="none";}}>×</button></div>)}<button onClick={add} style={{background:"none",border:`1.5px dashed ${P.border}`,borderRadius:10,color:P.accent,fontSize:12,fontFamily:"'Plus Jakarta Sans'",padding:"9px 0",cursor:"pointer",fontWeight:600,transition:"all .2s"}} onMouseEnter={e=>{e.target.style.borderColor=P.accent;e.target.style.background=P.accentLight;}} onMouseLeave={e=>{e.target.style.borderColor=P.border;e.target.style.background="none";}}>+ {addLabel}</button></div>;}
function Sec({title,children,open:dO=true,badge}){const[open,setOpen]=useState(dO);return<div style={{borderBottom:`1px solid ${P.borderLight}`,paddingBottom:open?4:0}}><button onClick={()=>setOpen(!open)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",background:"none",border:"none",color:P.text,cursor:"pointer",padding:"16px 0 10px",fontSize:13,fontWeight:700,fontFamily:"'Plus Jakarta Sans'",letterSpacing:"0.01em"}}><span style={{display:"flex",alignItems:"center",gap:8}}>{title}{badge>0&&<span style={{fontSize:11,fontWeight:600,color:P.accent,background:P.accentLight,padding:"2px 8px",borderRadius:20}}>{money(badge)}/mo</span>}</span><svg width="16" height="16" viewBox="0 0 16 16" style={{transform:open?"rotate(180deg)":"rotate(0deg)",transition:"transform .25s",color:P.textMute}}><path d="M4 6l4 4 4-4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg></button>{open&&<div style={{paddingBottom:16,display:"flex",flexDirection:"column",gap:14}}>{children}</div>}</div>;}
function KPI({label,value,sub,accent,icon}){const bg=accent===P.green?P.greenBg:accent===P.red?P.redBg:accent===P.amber?P.amberBg:accent===P.purple?P.purpleBg:P.accentLight;return<div style={{flex:1,background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:"20px 22px",display:"flex",flexDirection:"column",gap:6,minWidth:160,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><div style={{display:"flex",alignItems:"center",gap:6}}>{icon&&<span style={{width:28,height:28,borderRadius:8,background:bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{icon}</span>}<span style={{fontSize:11,fontWeight:600,color:P.textMute,letterSpacing:"0.04em",textTransform:"uppercase"}}>{label}</span></div><span style={{fontSize:28,fontWeight:700,fontFamily:"'Fraunces',serif",color:accent||P.text,letterSpacing:"-0.02em",lineHeight:1.1}}>{value}</span>{sub&&<span style={{fontSize:12,color:P.textSec}}>{sub}</span>}</div>;}
function Row({label,amount,highlight,green,red}){const col=highlight?P.text:green?P.green:red?P.red:P.textSec;return<div style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:`1px solid ${P.borderLight}`}}><span style={{fontSize:13,color:highlight?P.text:P.textSec,fontWeight:highlight?700:500}}>{label}</span><span style={{fontSize:14,fontWeight:highlight?700:600,color:col,fontVariantNumeric:"tabular-nums",fontFamily:"'Plus Jakarta Sans'"}}>{green&&amount>0?`-${money(amount)}`:signedMoney(amount)}</span></div>;}
function ScenarioMgr({state,onLoad,onReset,scenarios,setScenarios,onSave}){const[name,setName]=useState("");const[showSave,setShowSave]=useState(false);const[toast,setToast]=useState(null);const flash=(msg,color)=>{setToast({msg,color});setTimeout(()=>setToast(null),2500);};const save=(oN)=>{const n=(oN||name).trim();if(!n)return;const rest=scenarios.filter(s=>s.name!==n);if(!scenarios.find(s=>s.name===n)&&rest.length>=MX){flash(`Max ${MX} scenarios`,"#dc2626");return;}const upd=[...rest,{name:n,state:JSON.parse(JSON.stringify(state)),savedAt:Date.now()}];setScenarios(upd);onSave(upd);if(!oN){setName("");setShowSave(false);}flash(`"${n}" saved`,"#16a34a");};const del=n=>{const upd=scenarios.filter(s=>s.name!==n);setScenarios(upd);onSave(upd);flash(`"${n}" deleted`,"#6b6966");};return<div style={{padding:"14px 0",borderBottom:`1px solid ${P.borderLight}`}}>{toast&&<div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",background:P.bgCard,border:`1.5px solid ${toast.color}33`,color:toast.color,padding:"10px 24px",borderRadius:12,fontSize:13,fontFamily:"'Plus Jakarta Sans'",fontWeight:600,zIndex:9999,boxShadow:"0 8px 30px rgba(0,0,0,.12)"}}>{toast.msg}</div>}<div style={{display:"flex",gap:8,marginBottom:10}}><button onClick={()=>setShowSave(!showSave)} style={{border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontSize:12,fontWeight:600,borderRadius:10,padding:"8px 16px",background:P.accentLight,color:P.accent}}>💾 Save</button><button onClick={onReset} style={{border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontSize:12,fontWeight:600,borderRadius:10,padding:"8px 16px",background:P.redBg,color:P.red}}>↺ Clear</button></div>{showSave&&<div style={{display:"flex",gap:8,marginBottom:10}}><input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&save()} placeholder="e.g. Beach House $1.6M" maxLength={30} autoFocus style={{flex:1,background:P.bgInput,border:`1.5px solid ${P.border}`,borderRadius:10,color:P.text,fontSize:13,fontFamily:"'Plus Jakarta Sans'",padding:"8px 12px",outline:"none",fontWeight:500}}/><button onClick={()=>save()} style={{border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontSize:12,fontWeight:600,borderRadius:10,padding:"8px 18px",background:P.accent,color:"#fff"}}>Save</button></div>}{scenarios.length>0&&<div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:200,overflowY:"auto"}}><span style={{fontSize:11,color:P.textMute,fontWeight:600,letterSpacing:"0.04em",textTransform:"uppercase"}}>Saved ({scenarios.length}/{MX})</span>{scenarios.map(s=><div key={s.name} style={{display:"flex",alignItems:"center",gap:6,background:P.bgInput,borderRadius:10,padding:"7px 10px"}}><button onClick={()=>{onLoad(s.state);flash(`"${s.name}" loaded`,"#2563eb");}} style={{background:"none",border:"none",color:P.text,cursor:"pointer",fontSize:13,fontFamily:"'Plus Jakarta Sans'",textAlign:"left",flex:1,padding:0,fontWeight:500}}>{s.name}<span style={{fontSize:10,color:P.textMute,marginLeft:6}}>{(s.state.scenarioMode||"buy")==="rent"?"🏠rent":"🏡buy"}</span></button><button onClick={()=>save(s.name)} style={{border:"none",cursor:"pointer",background:P.accentLight,color:P.accent,borderRadius:6,padding:"3px 8px",fontSize:10,fontWeight:700,fontFamily:"'Plus Jakarta Sans'"}} title="Overwrite">↻</button><button onClick={()=>del(s.name)} style={{background:"none",border:"none",color:P.textMute,cursor:"pointer",fontSize:16,padding:"0 3px"}} onMouseEnter={e=>e.target.style.color=P.red} onMouseLeave={e=>e.target.style.color=P.textMute}>×</button></div>)}</div>}</div>;}
function CompareView({scenarios}){const[sel,setSel]=useState({});const[detail,setDetail]=useState(null);const ranked=useMemo(()=>scenarios.map(sc=>({...sc,calc:cFS(sc.state)})).sort((a,b)=>a.calc.delta-b.calc.delta),[scenarios]);const toggle=n=>setSel(p=>({...p,[n]:!p[n]}));const doExport=()=>{const e=ranked.filter(r=>sel[r.name]);if(!e.length){alert("Select scenarios first");return;}xls(e);};const cf=v=>v<0?P.red:v>0?P.green:P.textMute;if(!scenarios.length)return<div style={{padding:60,textAlign:"center",color:P.textMute,fontSize:14}}>Save at least one scenario to compare.</div>;return<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:8}}><div><h2 style={{fontSize:18,fontWeight:700,color:P.text,margin:0,fontFamily:"'Fraunces',serif"}}>Compare Scenarios</h2><p style={{fontSize:13,color:P.textSec,margin:"4px 0 0"}}>Ranked by monthly delta (net cost change) — best to worst. Lower delta = better deal.</p></div><button onClick={doExport} style={{border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans'",fontSize:12,fontWeight:600,borderRadius:10,padding:"8px 18px",background:P.greenBg,color:P.green}}>📊 Export CSV</button></div><div style={{overflowX:"auto",marginBottom:24,background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13,fontFamily:"'Plus Jakarta Sans'"}}><thead><tr style={{borderBottom:`2px solid ${P.border}`}}>{["","#","Scenario","Mode","Housing","Offsets","Delta",""].map((h,i)=><th key={i} style={{padding:"12px 10px",textAlign:i>=4&&i<=6?"right":i===7?"center":"left",color:P.textMute,fontWeight:700,fontSize:11,textTransform:"uppercase",letterSpacing:"0.04em"}}>{h}</th>)}</tr></thead><tbody>{ranked.map((r,idx)=><tr key={r.name} style={{borderBottom:`1px solid ${P.borderLight}`,background:detail===r.name?P.bgInput:"transparent"}}><td style={{padding:"10px"}}><input type="checkbox" checked={!!sel[r.name]} onChange={()=>toggle(r.name)} style={{accentColor:P.accent,width:16,height:16}}/></td><td style={{padding:"10px",color:P.textMute,fontWeight:600}}>#{idx+1}</td><td style={{padding:"10px",color:P.text,fontWeight:600}}>{r.name}</td><td style={{padding:"10px",color:P.textSec,fontSize:11}}>{(r.state.scenarioMode||"buy")==="rent"?"🏠 Rent":"🏡 Buy"}</td><td style={{padding:"10px",textAlign:"right",color:P.textSec,fontWeight:500}}>{money(r.calc.newHousing)}</td><td style={{padding:"10px",textAlign:"right",color:P.green,fontWeight:600}}>-{money(r.calc.totalOff)}</td><td style={{padding:"10px",textAlign:"right",fontWeight:700,color:cf(-r.calc.delta)}}>{signedMoney(r.calc.delta)}</td><td style={{padding:"10px",textAlign:"center"}}><button onClick={()=>setDetail(detail===r.name?null:r.name)} style={{background:P.accentLight,border:"none",color:P.accent,cursor:"pointer",fontSize:11,fontFamily:"'Plus Jakarta Sans'",fontWeight:600,borderRadius:6,padding:"4px 10px"}}>{detail===r.name?"Hide":"View"}</button></td></tr>)}</tbody></table></div>{detail&&(()=>{const sc=ranked.find(r=>r.name===detail);if(!sc)return null;const c=sc.calc;return<div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:24,marginBottom:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><h3 style={{fontSize:16,fontWeight:700,color:P.text,marginBottom:20,fontFamily:"'Fraunces',serif"}}>{sc.name} <span style={{fontSize:12,color:P.textSec,fontWeight:500}}>{(sc.state.scenarioMode||"buy")==="rent"?"(Rental)":"(Purchase)"}</span></h3><div style={{display:"flex",gap:24,flexWrap:"wrap"}}><div style={{flex:1,minWidth:200}}><div style={{fontSize:11,fontWeight:700,color:P.textMute,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:10}}>New Costs</div>{c.newBreakdown.filter(r=>r.amount>0).map((r,i)=><Row key={i} label={r.label} amount={r.amount}/>)}<Row label="Total" amount={c.newHousing} highlight/></div><div style={{flex:1,minWidth:200}}><div style={{fontSize:11,fontWeight:700,color:P.textMute,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:10}}>Offsets</div>{c.offBreakdown.filter(r=>r.amount>0).map((r,i)=><Row key={i} label={r.label} amount={r.amount} green/>)}<Row label="Total" amount={c.totalOff} highlight/></div></div></div>;})()}</div>;}
function EquivView({scenarios}){
  ...
  const mx=calcs.map(a=>calcs.map(b=>{
    if(a.name===b.name)return null;
    const bMode=b.state.scenarioMode||"buy";
    if(bMode==="buy")return{type:"buy",val:eqHP(a.calc.delta,b.state),ref:b.state.homePrice};
    else return{type:"rent",val:eqRent(a.calc.delta,b.state),ref:b.state.monthlyRent};
  }));
  return<div>
    <div style={{marginBottom:16}}><h2 style={{fontSize:18,fontWeight:700,color:P.text,margin:0,fontFamily:"'Fraunces',serif"}}>Equivalency Matrix</h2><p style={{fontSize:13,color:P.textSec,margin:"4px 0 0"}}>Each cell answers: "To match Row's monthly delta using Column's scenario, what price/rent would you need?" <strong style={{color:P.green}}>Green</strong> = you can afford more, <strong style={{color:P.red}}>Red</strong> = you can afford less, compared to Column's current price/rent.</p></div>
    <div style={{overflowX:"auto",background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      <table style={{borderCollapse:"collapse",fontSize:12,fontFamily:"'Plus Jakarta Sans'"}}>
        <thead><tr>
          ...
          {calcs.map(c=><th key={c.name} style={{padding:"12px",textAlign:"right",color:P.textSec,fontSize:12,fontWeight:700,borderBottom:`2px solid ${P.border}`,whiteSpace:"nowrap"}}>{c.name}<div style={{fontSize:10,color:P.textMute,fontWeight:500}}>{(c.state.scenarioMode||"buy")==="buy"?`🏡 ${money(c.state.homePrice)}`:`🏠 ${money(c.state.monthlyRent)}/mo`}</div></th>)}
        </tr></thead>
        <tbody>{calcs.map((a,i)=><tr key={a.name} style={{borderBottom:`1px solid ${P.borderLight}`}}>
          ...
          {calcs.map((b,j)=>{const cell=mx[i][j];if(!cell)return<td key={b.name} style={{padding:"10px 12px",textAlign:"right",color:P.borderLight,fontWeight:600}}>—</td>;
            const diff=cell.val-cell.ref;const better=diff>0;
            return<td key={b.name} style={{padding:"10px 12px",textAlign:"right",color:P.text,fontWeight:600,background:better?P.greenBg:P.redBg}}>
              {cell.type==="rent"?`${money(cell.val)}/mo`:money(cell.val)}
              <div style={{fontSize:10,fontWeight:600,color:better?P.green:P.red}}>{better?"+":""}{cell.type==="rent"?`${signedMoney(diff)}/mo`:signedMoney(diff)}</div>
            </td>;
          })}
        </tr>)}</tbody>
      </table>
...
function SidebarContent({s,set,sideTab,setSideTab}){
  const isBuy=(s.scenarioMode||"buy")==="buy";
  const chm=s.currentHouseMode||"sell";
  return<>
    <div style={{marginBottom:12}}><label style={{fontSize:12,fontWeight:700,color:P.textSec,display:"block",marginBottom:6}}>Scenario Type</label><Toggle options={[{value:"buy",label:"🏡 Buy"},{value:"rent",label:"🏠 Rent"}]} value={s.scenarioMode||"buy"} onChange={v=>set("scenarioMode",v)}/></div>
    <div style={{display:"flex",margin:"10px 0",gap:2,background:P.bgInput,borderRadius:12,padding:3,border:`1.5px solid ${P.border}`}}>
      {[[isBuy?"mortgage":"rental",isBuy?"Mortgage":"Rental"],["offsets","Offsets"],["income","Income"],["expenses","Expenses"]].map(([k,l])=><button key={k} onClick={()=>setSideTab(k)} style={{flex:1,background:sideTab===k?P.bgCard:"transparent",border:"none",borderRadius:9,color:sideTab===k?P.text:P.textMute,cursor:"pointer",padding:"9px 0",fontSize:11,fontWeight:700,fontFamily:"'Plus Jakarta Sans'",transition:"all .2s",boxShadow:sideTab===k?"0 1px 2px rgba(0,0,0,.06)":"none"}}>{l}</button>)}
    </div>
    {sideTab==="mortgage"&&isBuy&&<><Sec title="Purchase Details"><div style={{display:"flex",gap:10}}><Field label="Home Price" prefix="$" value={s.homePrice} onChange={v=>set("homePrice",v)}/><Field label="Down Payment" prefix="$" value={s.downPayment} onChange={v=>set("downPayment",v)}/></div><div style={{display:"flex",alignItems:"flex-end",gap:10}}><div style={{flex:1,display:"flex",flexDirection:"column",gap:5}}><label style={{fontSize:12,fontWeight:600,color:P.textSec}}>Type</label><Toggle options={[{value:"30fixed",label:"30yr Fixed"},{value:"io",label:"7/1 IO"}]} value={s.mortgageType} onChange={v=>set("mortgageType",v)}/></div><Field label="Rate" suffix="%" value={s.rate} onChange={v=>set("rate",v)} small/></div><div style={{display:"flex",gap:10}}><Field label="HOA" prefix="$" suffix="/mo" value={s.hoa} onChange={v=>set("hoa",v)}/><Field label="PMI" prefix="$" suffix="/mo" value={s.pmi} onChange={v=>set("pmi",v)}/></div></Sec>
    <Sec title="Taxes & Insurance"><div style={{display:"flex",flexDirection:"column",gap:5}}><label style={{fontSize:12,fontWeight:600,color:P.textSec}}>Property Tax</label><Toggle options={[{value:"dollar",label:"$/year"},{value:"pct",label:"% of price"}]} value={s.taxMode} onChange={v=>set("taxMode",v)}/></div>{s.taxMode==="dollar"?<Field label="Annual Property Tax" prefix="$" suffix="/yr" value={s.annualTax} onChange={v=>set("annualTax",v)}/>:<Field label="Tax Rate" suffix="%" value={s.taxRatePct} onChange={v=>set("taxRatePct",v)} hint={s.homePrice>0?`= ${money(s.homePrice*s.taxRatePct/100)}/yr`:""}/>}<Field label="Annual Insurance" prefix="$" suffix="/yr" value={s.annualInsurance} onChange={v=>set("annualInsurance",v)}/></Sec>
    <Sec title="Monthly Add-ons" open={false}><DynList items={s.addons||[]} setItems={v=>set("addons",v)} defaultName="Add-on" addLabel="Add cost"/></Sec></>}
    {sideTab==="rental"&&!isBuy&&<><Sec title="Rental Costs"><Field label="Monthly Rent" prefix="$" suffix="/mo" value={s.monthlyRent} onChange={v=>set("monthlyRent",v)}/><div style={{display:"flex",gap:10}}><Field label="Renter's Insurance" prefix="$" suffix="/mo" value={s.renterInsurance} onChange={v=>set("renterInsurance",v)}/><Field label="Pet Fee" prefix="$" suffix="/mo" value={s.petFee} onChange={v=>set("petFee",v)}/></div><div style={{display:"flex",gap:10}}><Field label="Landscaping" prefix="$" suffix="/mo" value={s.rentLandscaping} onChange={v=>set("rentLandscaping",v)}/><Field label="HOA" prefix="$" suffix="/mo" value={s.rentHoa} onChange={v=>set("rentHoa",v)}/></div><Field label="Parking" prefix="$" suffix="/mo" value={s.rentParking} onChange={v=>set("rentParking",v)}/></Sec>
    <Sec title="Other Rental Costs" open={false}><DynList items={s.rentAddons||[]} setItems={v=>set("rentAddons",v)} defaultName="Cost" addLabel="Add cost"/></Sec></>}
    ...
    <Sec title={<>FICA Cap Savings<Tip text="If your salary exceeds the Social Security wage base, you stop paying FICA tax (6.2%) on income above the cap."/></>}><Toggle options={[{value:"dollar",label:"$/mo"},{value:"pct",label:"% salary"}]} value={s.ficaMode} onChange={v=>set("ficaMode",v)}/>{s.ficaMode==="dollar"?<Field label="FICA Savings" prefix="$" suffix="/mo" value={s.ficaDollar} onChange={v=>set("ficaDollar",v)}/>:<><Field label="FICA Rate" suffix="%" value={s.ficaPct} onChange={v=>set("ficaPct",v)}/><Field label="Base Salary" prefix="$" suffix="/yr" value={s.ficaBaseSalary} onChange={v=>set("ficaBaseSalary",v)} hint={s.ficaBaseSalary>0?`= ${money((s.ficaPct/100)*s.ficaBaseSalary/12)}/mo`:""}/></>}</Sec>
    <Sec title="Car Payoffs"><DynList items={s.carPayoffs||[]} setItems={v=>set("carPayoffs",v)} defaultName="Car" addLabel="Add car"/></Sec>
    <Sec title="Other Offsets"><DynList items={s.otherOffsets||[]} setItems={v=>set("otherOffsets",v)} defaultName="Offset" addLabel="Add offset"/></Sec></>}
    {sideTab==="income"&&<Sec title="Household Income"><Field label="Combined Monthly Take-Home" prefix="$" suffix="/mo" value={s.monthlyIncome} onChange={v=>set("monthlyIncome",v)} hint="After-tax income"/></Sec>}
    {sideTab==="expenses"&&<><div style={{padding:"10px 0 6px",fontSize:13,color:P.textSec,lineHeight:1.5}}>Ongoing expenses that <strong style={{color:P.amber}}>persist regardless</strong>.</div>{CATS.map(cat=><Sec key={cat.key} title={`${cat.icon} ${cat.label}`} open={false} badge={sm((s.expenses||{})[cat.key]||[])}><DynList items={(s.expenses||{})[cat.key]||[]} setItems={items=>set("expenses",{...(s.expenses||eE()),[cat.key]:items})} defaultName={cat.label} addLabel={`Add ${cat.label.toLowerCase()}`}/></Sec>)}</>}
  </>;
}
/* ── Timeline Planner ── */
    ...
    for(let yr=1;yr<=maxYear;yr++){
      ...
      for(const ev of yrEvents){
        if(ev.type==="income_change") runIncome+=(ev.amount||0);
          ...
          if(isBuy){
            const loan=Math.max(0,(baseState.homePrice||0)-(baseState.downPayment||0));
            const newPI=baseState.mortgageType==="30fixed"?fPI(loan,runRate,30):ioP(loan,runRate);
            const oldPI=baseCalc.pi||0;
            runHousing=runHousing-oldPI+newPI;
          }
        }
        else if(ev.type==="custom"){
          if((ev.amount||0)>0)extraItems.push({label:ev.label||"Custom",amount:ev.amount||0,endYear:ev.endYear||null});
          else runIncome+=(ev.amount||0);
        }
      }
      /* remove expired extras */
      extraItems=extraItems.filter(x=>!x.endYear||yr<=x.endYear);
  ...
  return<div>
    ...
    {events.length>0&&<div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:20,marginBottom:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {events.map(ev=><div key={ev.id} style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",padding:"10px 12px",background:P.bgInput,borderRadius:12}}>
          <select value={ev.year} onChange={e=>upEvent(ev.id,"year",Number(e.target.value))} style={{background:P.bgCard,border:`1.5px solid ${P.border}`,borderRadius:8,color:P.text,fontSize:12,fontFamily:"'Plus Jakarta Sans'",padding:"6px 8px",outline:"none",fontWeight:600,width:70}}>
            {Array.from({length:maxYear},(_,i)=>i+1).map(y=><option key={y} value={y}>Yr {y}</option>)}
          </select>
          <select value={ev.type} onChange={e=>upEvent(ev.id,"type",e.target.value)} style={{background:P.bgCard,border:`1.5px solid ${P.border}`,borderRadius:8,color:P.text,fontSize:12,fontFamily:"'Plus Jakarta Sans'",padding:"6px 8px",outline:"none",fontWeight:500,width:140}}>
            {eventTypes.map(t=><option key={t.value} value={t.value}>{t.icon} {t.label}</option>)}
          </select>
          <input value={ev.label||""} onChange={e=>upEvent(ev.id,"label",e.target.value)} placeholder={ev.type==="expense_end"?"Which expense?":"Description"} style={{flex:1,minWidth:100,background:P.bgCard,border:`1.5px solid ${P.border}`,borderRadius:8,color:P.text,fontSize:12,fontFamily:"'Plus Jakarta Sans'",padding:"6px 10px",outline:"none",fontWeight:500}}/>
          <div style={{display:"flex",alignItems:"center",background:P.bgCard,borderRadius:8,border:`1.5px solid ${P.border}`,overflow:"hidden",width:110,height:34}}>
            <span style={{padding:"0 0 0 8px",color:P.textMute,fontSize:12,fontWeight:600}}>{ev.type==="rate_change"?"%":"$"}</span>
            <NumInput value={ev.amount||0} onChange={v=>upEvent(ev.id,"amount",v)} style={{fontSize:12,padding:"0 6px"}}/>
          </div>
          {(ev.type==="expense_add"||ev.type==="custom")&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:11,color:P.textMute}}>ends yr</span><select value={ev.endYear||""} onChange={e=>upEvent(ev.id,"endYear",e.target.value?Number(e.target.value):null)} style={{background:P.bgCard,border:`1.5px solid ${P.border}`,borderRadius:8,color:P.text,fontSize:11,fontFamily:"'Plus Jakarta Sans'",padding:"4px 6px",outline:"none",width:60}}>
    ...
    <div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:24,marginBottom:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)",overflowX:"auto"}}>
      ...
      <div style={{display:"flex",gap:16,marginTop:8,fontSize:11,color:P.textMute}}>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:12,height:3,background:P.accent,borderRadius:2,display:"inline-block"}}/>Surplus</span>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:P.green,display:"inline-block"}}/>Positive</span>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:P.red,display:"inline-block"}}/>Negative</span>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:12,height:0,borderTop:`2px dashed ${P.amber}`,display:"inline-block"}}/>Event</span>
      </div>
    </div>
    ...
    <div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:0,boxShadow:"0 1px 3px rgba(0,0,0,.04)",overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,fontFamily:"'Plus Jakarta Sans'"}}>
        ...
        <tbody>{yearData.map(d=><tr key={d.year} style={{borderBottom:`1px solid ${P.borderLight}`,background:d.events.length>0?P.amberBg:"transparent"}}>
          ...
          <td style={{padding:"10px",fontSize:11,color:P.textSec,maxWidth:200}}>{d.events.length>0?d.events.map(e=>`${e.label||e.type}${e.amount?` (${e.type==="rate_change"?e.amount+"%":"$"+Math.abs(e.amount).toLocaleString()})`:""}`).join(", "):"—"}</td>
        </tr>)}</tbody>
      </table>
    </div>
  </div>;
...
export default function App(){
  ...
  const[state,setState]=useState(gD());
  const set=(k,v)=>setState(p=>({...p,[k]:v}));
  const[sideTab,setSideTab]=useState("mortgage");
  ...
  // Fix sideTab when mode switches
  useEffect(()=>{const isBuy=(state.scenarioMode||"buy")==="buy";if(sideTab==="mortgage"&&!isBuy)setSideTab("rental");if(sideTab==="rental"&&isBuy)setSideTab("mortgage");},[state.scenarioMode]);

  useEffect(()=>{if(isAuth){setSyncStatus("Syncing...");loadCloud().then(cloud=>{if(cloud&&cloud.length>0){setScenarios(cloud);saveLocal(cloud);setSyncStatus("✓");}else setSyncStatus("");setTimeout(()=>setSyncStatus(""),3000);}).catch(()=>setSyncStatus(""));}},[isAuth]);
  ...
  const loadState=s=>{setState({...gD(),...s});};
  const resetAll=()=>setState(gD());
  const calc=useMemo(()=>cFS(state),[state]);
  const dC=calc.delta>0?P.red:calc.delta<0?P.green:P.textMute;
  const cfC=v=>v>0?P.green:v<0?P.red:P.textMute;
  const viewTabs=[{v:"delta",l:"Delta",i:"📊"},{v:"cashflow",l:"Cashflow",i:"💰"},{v:"cv",l:"Current vs New",i:"⚖️"},{v:"tl",l:"Timeline",i:"📅"},{v:"sc",l:"Compare",i:"📋"},{v:"eq",l:"Equiv",i:"🔄"}];
  const mobileTabs=[{v:"inputs",l:"Inputs",i:"✏️"},{v:"delta",l:"Delta",i:"📊"},{v:"cashflow",l:"Cash",i:"💰"},{v:"cv",l:"Cur/New",i:"⚖️"},{v:"tl",l:"Timeline",i:"📅"},{v:"sc",l:"Compare",i:"📋"},{v:"eq",l:"Equiv",i:"🔄"}];
  const onMobileTab=(v)=>{setMobileTab(v);if(v!=="inputs")setView(v);};
  const isBuy=(state.scenarioMode||"buy")==="buy";
  const modeLabel=isBuy?"Purchase":"Rental";
  ...
  return<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",background:P.bg,color:P.text,minHeight:"100vh"}}>
    ...
    <div className={`layout ${mobileTab==="inputs"?"mob-hide-main":"mob-show-main"}`} style={{display:"flex",minHeight:"calc(100vh - 57px)"}}>
      <div className="sidebar" style={{width:380,background:P.bgSide,borderRight:`1px solid ${P.border}`,overflowY:"auto",maxHeight:"calc(100vh - 57px)",position:"sticky",top:57,padding:"4px 22px 40px"}}>
        <ScenarioMgr state={state} onLoad={loadState} onReset={resetAll} scenarios={scenarios} setScenarios={setScenarios} onSave={persistScenarios}/>
        <SidebarContent s={state} set={set} sideTab={sideTab} setSideTab={setSideTab}/>
      </div>
      <div className="main" style={{flex:1,padding:"28px 40px",overflowY:"auto",maxWidth:960,margin:"0 auto"}}>
        {view==="delta"&&<><InfoBanner color="blue">📊 <strong>Mortgage Delta</strong> shows the net change in your monthly housing costs. New {modeLabel.toLowerCase()} costs minus offsets (expenses that go away) = your monthly delta.</InfoBanner>
          <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:28}}><KPI label={`New ${modeLabel}`} value={money(calc.newHousing)} sub="before offsets" accent={P.accent} icon={isBuy?"🏡":"🏠"}/><KPI label="Offsets" value={money(calc.totalOff)} sub="eliminated" accent={P.green} icon="✅"/><KPI label="Monthly Delta" value={`${calc.delta>=0?"":"-"}${money(calc.delta)}`} sub={calc.delta>0?"more/mo":calc.delta<0?"less/mo":"neutral"} accent={dC} icon={calc.delta>0?"📈":"📉"}/></div>
          {calc.newHousing===0&&<div style={{padding:"16px 20px",borderRadius:14,marginBottom:28,fontSize:14,background:P.amberBg,color:P.amber,border:`1.5px solid #fcd34d`,fontWeight:500}}>Enter your scenario details in the sidebar to get started.</div>}
          <div style={{display:"flex",gap:28,flexWrap:"wrap",marginBottom:28}}>
            <div style={{flex:1,minWidth:260}}><div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h3 style={{fontSize:15,fontWeight:700,margin:0}}>New Costs</h3><span style={{fontSize:20,fontFamily:"'Fraunces',serif",fontWeight:700,color:P.accent}}>{money(calc.newHousing)}</span></div><HBar items={calc.newBreakdown} total={calc.newHousing}/><div style={{marginTop:16}}>{calc.newBreakdown.filter(r=>r.amount>0).map((r,i)=><Row key={i} label={r.label} amount={r.amount}/>)}<Row label="Total" amount={calc.newHousing} highlight/></div></div></div>
            <div style={{flex:1,minWidth:260}}><div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h3 style={{fontSize:15,fontWeight:700,margin:0}}>Offsets</h3><span style={{fontSize:20,fontFamily:"'Fraunces',serif",fontWeight:700,color:P.green}}>-{money(calc.totalOff)}</span></div><HBar items={calc.offBreakdown.filter(r=>r.amount>0)} total={calc.totalOff}/><div style={{marginTop:16}}>{calc.offBreakdown.filter(r=>r.amount>0).map((r,i)=><Row key={i} label={r.label} amount={r.amount} green/>)}<Row label="Total" amount={calc.totalOff} highlight/></div></div></div>
          </div>
          <div style={{padding:"24px 28px",borderRadius:16,background:P.bgCard,border:`1.5px solid ${dC}22`,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><div><div style={{fontSize:11,fontWeight:700,color:P.textMute,letterSpacing:"0.04em",textTransform:"uppercase",marginBottom:4}}>Net Monthly Change<Tip text="Total new housing costs minus all offsets. Positive = more expensive. Negative = you save money."/></div><div style={{fontSize:13,color:P.textSec}}>{money(calc.newHousing)} − {money(calc.totalOff)} offsets</div></div><span style={{fontSize:38,fontWeight:700,fontFamily:"'Fraunces',serif",color:dC}}>{calc.delta>=0?"":"−"}{money(calc.delta)}<span style={{fontSize:14,color:P.textMute,marginLeft:6,fontFamily:"'Plus Jakarta Sans'",fontWeight:500}}>/mo</span></span></div>
          {calc.postIO&&calc.newHousing>0&&<div style={{marginTop:12,padding:"12px 16px",borderRadius:12,background:P.amberBg,fontSize:13,color:P.amber,fontWeight:500}}>⚠ Post-IO estimate: ~{money(calc.postIO)}/mo</div>}
        </>}

        {view==="cashflow"&&<><InfoBanner color="green">💰 <strong>Full Cashflow</strong> shows income minus all outflows (housing + ongoing expenses). Unlike Delta, this shows your actual monthly surplus or shortfall.</InfoBanner>
          <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:28}}><KPI label="Income" value={money(calc.monthlyIncome)} accent={P.accent} icon="💵"/><KPI label="Housing" value={money(calc.newHousing)} accent={P.amber} icon={isBuy?"🏡":"🏠"}/><KPI label="Ongoing" value={money(calc.totalOngoing)} accent={P.purple} icon="📋"/><KPI label="Surplus" value={signedMoney(calc.newCashflow)} sub={calc.newCashflow>=0?"remaining":"shortfall"} accent={cfC(calc.newCashflow)} icon={calc.newCashflow>=0?"✅":"⚠️"}/></div>
          ...
          <div style={{display:"flex",gap:28,flexWrap:"wrap",alignItems:"flex-start",marginBottom:28}}>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16}}><Donut items={[{label:"Housing",amount:calc.newHousing},...calc.expByCat.filter(c=>c.total>0).map(c=>({label:c.label,amount:c.total}))]} label="Outflows" amount={money(calc.newTotalOut)} size={180} thickness={24}/></div>
            <div style={{flex:1,minWidth:260}}><div style={{background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><h3 style={{fontSize:15,fontWeight:700,margin:"0 0 16px"}}>Monthly Breakdown</h3><Row label="Income" amount={calc.monthlyIncome}/><div style={{height:6}}/><Row label="Housing (all-in)" amount={calc.newHousing} red/>{calc.expByCat.filter(c=>c.total>0).map(c=><Row key={c.key} label={`${c.icon} ${c.label}`} amount={c.total} red/>)}<Row label="Total Outflows" amount={calc.newTotalOut} highlight/></div></div>
          </div>
        ...
        {view==="cv"&&<><InfoBanner color="blue">⚖️ <strong>Current vs New</strong> compares your finances side-by-side. Current = your housing costs today. New = your {modeLabel.toLowerCase()} costs after the move.</InfoBanner>
          ...
          <div style={{display:"flex",gap:24,flexWrap:"wrap",marginBottom:28}}>
            <div style={{flex:1,minWidth:280,background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}><h3 style={{fontSize:15,fontWeight:700,margin:0}}>Current</h3><span style={{fontSize:11,color:P.textMute,fontWeight:600,textTransform:"uppercase",background:P.bgInput,padding:"4px 10px",borderRadius:8}}>Before</span></div><Row label="Income" amount={calc.monthlyIncome}/><div style={{height:6}}/><Row label="Housing" amount={calc.curHousingExp} red/><Row label="State Tax" amount={calc.stateTaxSavings} red/>{calc.expByCat.filter(c=>c.total>0).map(c=><Row key={c.key} label={`${c.icon} ${c.label}`} amount={c.total} red/>)}<Row label="Total Out" amount={calc.currentTotalOut} highlight/><div style={{height:8}}/><div style={{display:"flex",justifyContent:"space-between",padding:"12px 0",borderTop:`2px solid ${P.border}`}}><span style={{fontSize:15,fontWeight:700}}>Surplus</span><span style={{fontSize:22,fontWeight:700,fontFamily:"'Fraunces',serif",color:cfC(calc.currentCashflow)}}>{signedMoney(calc.currentCashflow)}</span></div></div>
    ...
    <div className="mob-bottom" style={{display:"none",position:"fixed",bottom:0,left:0,right:0,background:P.bgCard,borderTop:`1px solid ${P.border}`,padding:"6px 8px env(safe-area-inset-bottom,0px)",justifyContent:"space-around",zIndex:100,boxShadow:"0 -2px 12px rgba(0,0,0,.08)"}}>
      {mobileTabs.map(t=><button key={t.v} onClick={()=>onMobileTab(t.v)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,border:"none",background:"none",cursor:"pointer",padding:"6px 2px",minWidth:48,borderRadius:8,transition:"all .2s",backgroundColor:mobileTab===t.v?P.accentLight:"transparent"}}>
        <span style={{fontSize:16}}>{t.i}</span>
        <span style={{fontSize:9,fontWeight:700,color:mobileTab===t.v?P.accent:P.textMute,fontFamily:"'Plus Jakarta Sans'"}}>{t.l}</span>
      </button>)}
    </div>
  </div>;
}
