...
function EquivView({scenarios}){const calcs=useMemo(()=>scenarios.map(sc=>({...sc,calc:cFS(sc.state)})),[scenarios]);if(scenarios.length<2)return<div style={{padding:60,textAlign:"center",color:P.textMute,fontSize:14}}>Save 2+ scenarios for equivalencies.</div>;const mx=calcs.map(a=>calcs.map(b=>{if(a.name===b.name)return null;if((b.state.scenarioMode||"buy")!=="buy")return null;return eqHP(a.calc.delta,b.state);}));return<div><div style={{marginBottom:16}}><h2 style={{fontSize:18,fontWeight:700,color:P.text,margin:0,fontFamily:"'Fraunces',serif"}}>Equivalency Matrix</h2><p style={{fontSize:13,color:P.textSec,margin:"4px 0 0"}}>Each cell answers: "To have the same monthly delta as the Row scenario, what home price could you afford in the Column scenario's location?" <strong style={{color:P.green}}>Green</strong> = more house, <strong style={{color:P.red}}>Red</strong> = less house vs Row's price. Only Buy scenarios appear as columns.</p></div><div style={{overflowX:"auto",background:P.bgCard,borderRadius:16,border:`1px solid ${P.border}`,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><table style={{borderCollapse:"collapse",fontSize:12,fontFamily:"'Plus Jakarta Sans'"}}><thead><tr><th style={{padding:"12px 12px",textAlign:"left",color:P.textMute,fontSize:10,textTransform:"uppercase",fontWeight:700,letterSpacing:"0.04em",borderBottom:`2px solid ${P.border}`,position:"sticky",left:0,background:P.bgCard,zIndex:1}}>Match ↓ via →</th>{calcs.filter(c=>(c.state.scenarioMode||"buy")==="buy").map(c=><th key={c.name} style={{padding:"12px",textAlign:"right",color:P.textSec,fontSize:12,fontWeight:700,borderBottom:`2px solid ${P.border}`,whiteSpace:"nowrap"}}>{c.name}<div style={{fontSize:10,color:P.textMute,fontWeight:500}}>{money(c.state.homePrice)}</div></th>)}</tr></thead><tbody>{calcs.map((a,i)=>{const buyIdxs=calcs.map((c,j)=>({c,j})).filter(x=>(x.c.state.scenarioMode||"buy")==="buy");return<tr key={a.name} style={{borderBottom:`1px solid ${P.borderLight}`}}><td style={{padding:"10px 12px",color:P.text,fontWeight:600,whiteSpace:"nowrap",position:"sticky",left:0,background:P.bgCard,zIndex:1}}>{a.name}<div style={{fontSize:10,color:P.textMute,fontWeight:500}}>Δ {signedMoney(a.calc.delta)}/mo</div></td>{buyIdxs.map(({c:b,j})=>{const val=mx[i][j];return<td key={b.name} style={{padding:"10px 12px",textAlign:"right",color:val===null?P.borderLight:P.text,fontWeight:600,background:val===null?"transparent":val<a.state.homePrice?P.redBg:P.greenBg}}>{val===null?"—":money(val)}{val!==null&&<div style={{fontSize:10,fontWeight:600,color:val<a.state.homePrice?P.red:P.green}}>{val>a.state.homePrice?"+":""}{signedMoney(val-a.state.homePrice)}</div>}</td>;})}</tr>;})}</tbody></table></div></div>;}
